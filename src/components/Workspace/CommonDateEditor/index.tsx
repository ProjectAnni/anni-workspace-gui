...
const CommonDateEditor: React.FC<Props> = (props: Props) => {
    ...
    const onTextValueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (/\d{6}/.test(newDate)) {
            ...
            const autoParsedDate = `${+y1 < 8 ? "20" : "19"}${y1}${y2}-${m1}${m2}-${d1}${d2}`;
        ...
        const [_, year, month, date] = newDate.match(/(\d{4})(?:-(\d{2})(?:-(\d{2}))?)?/)!;
        if (year && month && date) {
    ...
    return (
        <FormGroup label="发售日期" labelInfo="(required)" helperText={helpText} intent={intent}>
            <InputGroup value={textValue} onChange={onTextValueChange} />
        </FormGroup>
    );
};

export default CommonDateEditor;

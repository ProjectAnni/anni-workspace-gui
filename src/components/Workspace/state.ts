...
export const WorkspaceRepoBasePathAtom = atom(async (get) =>
    path.resolve(get(WorkspaceBasePathAtom), ".anni/repo/")
);
...
export const WorkspaceRepoConfigAtom = atom(async (get) => {
    ...
    const parsedConfigFile =
        (TOML.parse(configFileContent)?.repo as unknown as RepoConfig) || {};
    ...
    for (const p of albumDirectories) {
        extendedConfig.albumPaths!.push(
            await path.resolve(basePath, `.anni/repo/${p}`)
        );
    }
